...
export default function ProblemWorkspace({ problemSlug, onBack }) {
  ...
  const handleSubmitSolution = async () => {
    ...
    setConsoleOutput({
      ...
      message: 'Running against full suite of hidden and sample test cases...'
    });
          ...
          <div className="p-6 overflow-y-auto space-y-6 text-slate-300 text-xs leading-relaxed flex-1">
            {activeTab === 'description' ? (
              ...
              <div className="text-center py-12 text-slate-500">
                No past submissions recorded for this problem yet.
              </div>
            )}
          </div>
        </div>
            ...
            <div className="p-3 overflow-y-auto flex-1 font-mono text-[11px] space-y-2 bg-slate-950 text-slate-300">
              {consoleOutput ? (
                <div className="space-y-1.5">
                  <div className="flex items-center space-x-2">
                    <span className={`font-bold ${getStatusColor(consoleOutput.status)}`}>
